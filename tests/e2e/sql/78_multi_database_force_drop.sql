...
CREATE TEMP VIEW _e2e75_routes AS
SELECT activity.pid
...
WHERE activity.datid = (SELECT database_oid FROM _e2e75_old)
    ...
    AND activity.query LIKE 'ROLLBACK%'
    AND NOT EXISTS (SELECT 1 FROM pg_locks AS locks
...
SELECT pg_temp.e2e75_wait($check$
    SELECT (SELECT count(*) FROM _e2e75_waiters) = 1
        AND (SELECT count(*) FROM _e2e75_routes) = 1
...
$check$, 'old SQL routed with no metadata locks, before its user connection',
    (SELECT submitted_at + interval '10 seconds' FROM _e2e75_old));
UPDATE _e2e75_old SET route_pid = (SELECT pid FROM _e2e75_routes);
...
-- FORCE kills the idle source connection; a reconnect must reject the reused name.
DROP DATABASE _e2e75_origin WITH (FORCE);
DO $$
BEGIN
    PERFORM pg_stat_clear_snapshot();
    IF EXISTS (SELECT 1 FROM pg_stat_activity WHERE pid = (SELECT route_pid FROM _e2e75_old)) OR
        EXISTS (SELECT 1 FROM pg_database WHERE oid = (SELECT database_oid FROM _e2e75_old)) THEN
        RAISE EXCEPTION 'TEST FAILED [force drop]: old database or source connection survived';
    END IF;
END $$;
CREATE DATABASE _e2e75_origin;
GRANT CONNECT ON DATABASE _e2e75_origin TO df_e2e_user;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM _e2e75_sql_history WHERE outcome_type = 'ActivityFailed'
        AND error_message LIKE 'Origin admission unavailable:%') OR
...
DROP VIEW _e2e75_sql_history, _e2e75_routes, _e2e75_waiters;
