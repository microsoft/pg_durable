...
BEGIN
    ...
    -- BUG: Empty result (0 rows) causes $var substitution to produce unquoted
    -- JSON in SQL context, yielding: SELECT {"rows":[],"row_count":0}
    -- which is a syntax error. Once fixed in substitute_all_with_options
    -- (src/types.rs), change this to expect 'completed'.
    IF status != 'failed' THEN
        RAISE EXCEPTION 'TEST FAILED [B5]: expected Failed (known bug: empty result $var substitution), got %', status;
    END IF;

    RAISE NOTICE 'PASSED [B5]: confirmed known bug — zero-row $var substitution produces invalid SQL (status=failed)';
END $$;
...
BEGIN
    ...
    -- BUG: Same empty-rows substitution bug as B5. DML without RETURNING
    -- produces {"rows":[],"row_count":1}, and $update_result is substituted
    -- unquoted. Once fixed in substitute_all_with_options (src/types.rs),
    -- change this to expect 'completed' and re-enable the assertions below.
    IF status != 'failed' THEN
        RAISE EXCEPTION 'TEST FAILED [B6]: expected Failed (known bug: empty result $var substitution), got %', status;
    END IF;
    ...
    -- Verify the DML actually ran (the UPDATE itself executes; it's the
    -- subsequent SELECT $update_result that fails)
    ...
    RAISE NOTICE 'PASSED [B6]: confirmed known bug — DML $var substitution produces invalid SQL (status=failed), but DML side effect committed';
END $$;
