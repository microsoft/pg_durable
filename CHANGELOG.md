...
- **Provider compatibility rejection:** a pre-v0.2.2 schema now puts the background worker into a bounded stand-down state instead of a CPU/query/log hot loop. Transient catalog read failures still retry with backoff. `df.start()`, `df.signal()`, and `df.cancel()` check the installed version on every engine operation, including cached-client calls, so stale `_worker_ready` state cannot enqueue work that no compatible worker can process. Floor prereleases such as v0.2.2-rc1 are rejected according to SemVer ordering.
...
- **Pre-v0.2.2 compatibility:** packages from this release forward contain no incoming upgrade paths from v0.1.1, v0.2.0, or v0.2.1, and the binary no longer carries the runtime shims that served those schemas (`login_role` inserts, pre-v0.2.0 global-variable queries, and v0.1.1 provider-object ownership conversion). Support is scoped to the `duroxide-pg` provider compatibility line, which starts at v0.2.2. The background worker now refuses to initialize against a pre-v0.2.2 schema rather than applying `duroxide-pg` migrations over retired `duroxide-pg-opt` state. If you are unexpectedly on a pre-v0.2.2 schema, reinstall a package at v0.2.5 or earlier to regain the migration chain, or follow the downstream process for the `duroxide-pg-opt` line — the older package's SQL files are overwritten on disk by this install. Git history retains the exact retired DDL.
  ...
  A database that originated on v0.1.1, now reports v0.2.2 or later, and still has extension-owned provider objects is also outside support; the retired ownership conversion is not restored. Back up anything needed for investigation, inventory dependent objects, then reset by running `DROP EXTENSION pg_durable` and `CREATE EXTENSION pg_durable` **in a single transaction** — the background worker recreates a missing provider schema idempotently, so a gap between the two statements can leave a non-extension-owned schema behind that makes `CREATE EXTENSION` fail with "schema already exists". This deletes durable instances, graph nodes, variables, execution history, provider state, and extension grants. Plain `DROP EXTENSION` can fail because BGW-created provider objects depend on the extension-owned schema; use `CASCADE` only after reviewing and accepting every additional dependent object PostgreSQL reports. Published open-source releases begin at v0.2.2, no known installation started on v0.1.1, and no population telemetry was available for this decision.
