...
Endpoints and mappings live in the origin database where the workflow is submitted
...
server, using the caller's mapping in the origin database. Missing keys
...
role needs `CONNECT` and the required local `df` metadata privileges in each
...
Dropping a satellite removes its local metadata. Identity validation and metadata
access share short transactions; **normal DROP does not wait for arbitrary
SQL/HTTP activities**. Fresh OID/UUID checks after resource waits and immediately
before dispatch reject stale queued work, including explicit remote SQL targets.
Admission is not atomic with DROP: a last-check-to-send race remains, and already
dispatched effects can finish after normal or forced removal. Engine
...
Ordinary PostgreSQL locks held by caller transactions or user statements still
apply; removing the separate activity-long guard does not bypass those locks.
...
SQL retains autocommit for control, default/explicit-self satellites and remote
targets, including `VACUUM` and concurrent index creation. Preflight identity
transactions end before business SQL and do not change its timeout/isolation
settings. Runtime shutdown and server-side cancellation/quiescence remain
[known limitations](docs/multi-database-installation.md#release-blockers).
No automatic drain or administrative quiesce API is provided.
...
| **Origin routes** | Satellite metadata connections, shared by activities and maintenance | `pg_durable.max_origin_connections` | 12 |
...
route reserves one origin connection slot; routes close afterward.
...
# Range: 2..1000. Each active route reserves 1 metadata slot.
...
The configured steady-epoch worker ceiling is `6 + 12 + 10 + 10 + 1 = 39`, not an idle allocation.
...
This formula does not certify old-task quiescence during control-runtime replacement;
see the [deferred lifetime guarantees](docs/multi-database-installation.md#release-blockers).
...
**Satellite drop:** Short metadata transactions protect validation and access on
one connection. DROP does not drain arbitrary SQL/HTTP activities. Fresh identity
checks reject stale admission, but do not close the last-check-to-send race or
stop already-dispatched SQL/HTTP. Engine records can remain until reconciliation.
...
requests runtime shutdown after detecting removal; the pinned runtime does not
guarantee a joined activity drain. Worker-created objects depend on
