...
printf "default_version = '0.2.8'\n" > "$out_dir$extension_dir/pg_durable.control"
printf 'install sql\n' > "$out_dir$extension_dir/pg_durable--0.2.8.sql"
printf 'upgrade sql\n' > "$out_dir$extension_dir/pg_durable--0.2.7--0.2.8.sql"
EOF
chmod +x "$FAKE_CARGO"
...
for test_case in 17:.so 18:.so 17:.dylib 18:.dylib; do
    ...
    test -f "$stage_dir/usr/share/postgresql/$major/extension/pg_durable--0.2.8.sql"
    test -f "$stage_dir/usr/share/postgresql/$major/extension/pg_durable--0.2.7--0.2.8.sql"
    ...
    test "$(file_mode "$stage_dir/usr/share/postgresql/$major/extension/pg_durable--0.2.8.sql")" = "644"
