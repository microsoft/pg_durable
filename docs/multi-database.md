...
A single invocation selects one SQL execution database. Starting with 0.2.9, multiple
...
- **Local worker access.** `pg_durable.worker_role` defaults to the `postgres` superuser. A custom role needs database-local `CONNECT`, `df` metadata rights, and access for origin-local HTTP privilege lookup. `BYPASSRLS` does not grant those privileges.
...
- Those target-selection columns predate 0.2.8. The 0.2.9 multi-database upgrade adds
...
Focused regressions include `14_database` (SQL targeting) and
`75_multi_database_lifecycle` (satellite lifecycle). These do not establish
[lifecycle acceptance](multi-database-installation.md#release-blockers).
...
- **Distributed transactions and instant drop cancellation:** There is no cross-database atomicity or DDL hook. Short identity-validated metadata transactions and fresh pre-dispatch checks reject stale admission, but do not stop already-dispatched effects or close the last-check-to-send race. Normal DROP does not drain arbitrary activities. Reconciliation is limited to confirmed removed origins, not unreachable databases. Control drop destroys all satellites' shared engine history.
...
`2` to `1000`, restart required), reserving one slot per active route. The control
