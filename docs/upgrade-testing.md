...
- **In-flight schedule caveat:** This release also changes `df.wait_for_schedule` inside loops to recompute from the orchestration's recorded clock each generation. That fixes a busy-loop bug, but changes the history shape for a wait-in-loop instance that replays across the upgrade; customers should drain or restart such instances if they encounter nondeterministic replay.
