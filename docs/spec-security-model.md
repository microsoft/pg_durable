...
- Catalogs live in the trusted origin installation, independently of the workflow's SQL target or HTTP JSON fields. One read-only consistent snapshot supplies endpoint configuration and all named bindings for an attempt. Satellite catalog connections check database OID and installation UUID after admission. The caller connection uses the shared SQL/catalog admission budget and closes before HTTP I/O; subsequent attempts start fresh snapshots.
- Immediately before send, a short metadata transaction revalidates source identity and HTTP `EXECUTE`. This catches revocation/removal during catalog waits without mixing secret snapshots. It does not atomically couple a remote send to source DDL or retract an accepted request.
