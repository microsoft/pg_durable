...
**Catalog options**: Servers require `auth_scheme`; `base_url` may be omitted only with `auth_scheme 'none'` for named-secret storage. A supplied URL retains all validation requirements, and HTTP endpoint execution always requires one. `header_name` is required only for header authentication. Schemes are `none` (no mapping required for endpoint authentication), `bearer` (`token` mapping option), `header` (`header_value`), `query` (`query_string`), and `managed-identity` (activity-local token acquisition without a mapping). Named bindings read individual `"secret.<key>"` mapping options, allowing native per-key addition, rotation and removal without rewriting other credentials. Values are opaque text, not JSON. The prefix is accepted only in user mappings and confers no endpoint-authentication or ambient-identity behavior; server options remain closed. See [Endpoint Credential Catalog](../USER_GUIDE.md#endpoint-credential-catalog). FDW creation authority uses a native grant separate from `df.grant_usage`.

**Managed identity**: Only superusers may create or alter managed-identity endpoints; execution also requires a superuser owner. An optional server `client_id` selects a user-assigned identity by UUID. The token resource is fixed by a supported HTTPS destination on port 443, never by workflow data or a free-form scope option. Token acquisition follows permission, destination and binding checks, releases the catalog slot first, and uses a separate bounded IMDS-compatible client. Cache entries are in-memory only, keyed by identity and resource, without stale-token fallback. The administrator-controlled provider GUC is startup-only; no provider is contacted for other authentication modes. See [Managed identity security](http-security.md#38-managed-identity).
...
- Static credential lookup uses `pg_user_mappings` over `connect_as_user(submitted_by)`, never the worker's privileged pool. Missing servers, mappings, options or masked values fail explicitly; there is no fallback to another role's mapping or an unauthenticated request.
...
Per-role HTTP credentials use native user mappings, not a `df.secrets` table. The extension installs `pg_durable_fdw`, its option validator and reference helpers; endpoint owners manage foreign servers and user mappings using native PostgreSQL DDL. The validator rejects unsupported options and authentication schemes. Managed-identity configuration requires a superuser and does not store tokens in the catalog.
...
- Managed-identity authentication adds no SQL objects or upgrade DDL beyond the endpoint catalog. Existing durable inputs and operation ordering are unchanged; the provider is consulted only during activity execution, never replay.
