...
4. **Rework monitoring functions** for per-user visibility
   ...
   - `df.metrics()`: Restricted to superusers only (security review Finding 6). Removed from `func_sigs` in `df.grant_usage()` and added superuser check inside the function. ✅ Implemented in v0.2.3.
...
- **`df.metrics()`**: Restricted to superusers only (security review Finding 6, v0.2.3). `df.metrics()` exposes system-wide aggregate counts (total instances, running/completed/failed counts, total executions and events) from the duroxide store without per-user filtering. Removed from `func_sigs` in `df.grant_usage()` so regular users do not receive EXECUTE, and added an in-function superuser check for defense-in-depth. Regular users should use `df.list_instances()` to view a summary of their own workflows. ✅
