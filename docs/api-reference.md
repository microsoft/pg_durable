...
role needs `CONNECT` and required `df` metadata rights in each origin, plus
...
Each active route reserves one metadata connection slot. Identity validation and
metadata access share a short transaction on that connection; no metadata relation
lock is retained across arbitrary SQL/HTTP or resource waits. Routes close after
use; there is no idle pool per database
...
No activity-long guard transaction disables server transaction/idle policies.
