...
/// Split a keyset cursor of the form `<created_at>|<instance_id>` into its parts.
...
/// List durable function instances with keyset (cursor) pagination.
///
/// Pages are ordered by `(created_at DESC, id DESC)` and use the
/// `idx_instances_created_at_desc_id` index for efficient scans. The
/// `after_cursor` is the `next_cursor` returned by the previous page (format
/// `<created_at>|<instance_id>`); pass `NULL` for the first page. `total_count`
/// reflects all rows visible to the caller for the given `status_filter`, and
/// `next_cursor` is `NULL` once the final page has been returned.
#[pg_extern(schema = "df")]
...
> {
    ...
    if has_more {
        // Drop the lookahead row so the page contains at most `limit_count` rows.
        user_instances.pop();
    }
